"""
Zenith Pro Feature Handlers — All /command handlers for Pro features.
"""
import html
import asyncio
...
from zenith_crypto_bot.repository import (
    SubscriptionRepo, PriceAlertRepo, WalletTrackerRepo, WatchlistRepo
)
...
async def cmd_alert(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Usage: /alert BTC above 100000  or  /alert ETH below 2000"""
...
async def cmd_portfolio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show full portfolio with live P&L."""
    user_id = update.effective_user.id
...
async def cmd_gas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    gwei = gas["gas_gwei"]
    if gwei < 15: level, color = "🟢 LOW", "Excellent time to trade."
    elif gwei < 30: level, color = "🟡 MODERATE", "Acceptable for most transactions."
    elif gwei < 60: level, color = "🟠 HIGH", "Consider waiting for lower fees."
    else: level, color = "🔴 VERY HIGH", "Delay non-urgent transactions."
...
# ==========================================
# 🛡️ REAL TOKEN SECURITY SCAN
# ==========================================

async def perform_real_audit(user_id: int, contract: str, msg, is_pro: bool):
    """Perform a real security scan using GoPlus API."""
    try:
        ...
        risk_score = 0
        risks = []
        if is_honeypot: risk_score += 40; risks.append("🔴 HONEYPOT DETECTED")
        if not is_open_source: risk_score += 15; risks.append("🟠 Contract not verified")
        if is_proxy: risk_score += 10; risks.append("🟡 Proxy contract (upgradeable)")
        if can_take_back: risk_score += 20; risks.append("🔴 Owner can reclaim ownership")
        if owner_change_balance: risk_score += 25; risks.append("🔴 Owner can modify balances")
        try:
            if float(buy_tax) > 0.1: risk_score += 10; risks.append(f"🟠 Buy tax: {float(buy_tax)*100:.1f}%")
            if float(sell_tax) > 0.1: risk_score += 15; risks.append(f"🔴 Sell tax: {float(sell_tax)*100:.1f}%")
        except: pass
...
async def show_new_pairs(msg, is_pro: bool):
    """Scan for newly created Uniswap V2 pairs."""
